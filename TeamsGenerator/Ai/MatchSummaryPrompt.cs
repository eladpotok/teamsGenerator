...
- `penaltyGoals` contains only verified penalty goals. Do not mention penalty_heavy_evening or penalty_scoring_run patterns; application code adds their verified aggregate facts to the finished report. When either aggregate pattern exists, do not state the total penalty count or a player's penalty total yourself.
